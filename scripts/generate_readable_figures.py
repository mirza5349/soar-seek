...
outputs. Enhanced for maximum readability (4 figures per twocolumn row).
"""
import os
import glob
import json
...
FIG = os.path.join(WS, "outputs/enhanced_publication_figures")
...
# Scale up figure base size slightly to avoid overlapping large text
SCALE = 1.6
COL_W = 2.5 * SCALE  

# Drastically scale up font sizes for 50% scale reduction in twocolumn
plt.rcParams.update({
    'font.family': 'serif', 'font.size': 20, 'axes.labelsize': 18,
    'axes.titlesize': 18, 'xtick.labelsize': 18, 'ytick.labelsize': 18,
    'legend.fontsize': 16, 'lines.linewidth': 2.5, 'lines.markersize': 8.0,
...
def newfig(*args, **kwargs):
    return plt.subplots(figsize=(COL_W, 2.5 * SCALE))
...
def trajectories(run):
    ...
    fig, ax = newfig(3.0)
    for i, r in routes.items():
        ax.plot([p[1] for p in r], [p[0] for p in r], '--', color='gray', lw=1.0, alpha=0.6, zorder=1)
    ...
    ax.scatter(hp['east_m'], hp['north_m'], marker='*', s=80, c='#999', edgecolors='k', linewidths=0.5, zorder=6)
    ax.scatter(inv['east_m'], inv['north_m'], marker='*', s=150, c='#d62728', edgecolors='k', linewidths=0.8, zorder=7)
    for i, tr in traces.items():
        c = UAV_COLORS[(i - 1) % 6]
        ax.plot(tr['east_m'].to_numpy(), tr['north_m'].to_numpy(), color=c, lw=1.5, alpha=0.8, zorder=3)
        for mode, st in [('THERMAL_EXPLOITATION', dict(color='#ff7f0e', lw=3.0)),
                         ('EVENT_INVESTIGATION', dict(color='#d62728', lw=3.0)),
                         ('GLIDE_RETURN', dict(color='#2ca02c', lw=2.0)),
                         ('LANDING', dict(color='#7f7f7f', lw=2.0))]:
        ...
        ax.scatter(tr['east_m'].iloc[0], tr['north_m'].iloc[0], marker='o', s=30, c=c, edgecolors='k', zorder=8)
        ax.scatter(tr['east_m'].iloc[-1], tr['north_m'].iloc[-1], marker='X', s=50, c=c, edgecolors='k', zorder=8)
    handles = [Line2D([], [], color='gray', ls='--', lw=1.5, label='Planned'),
               Line2D([], [], color='#ff7f0e', lw=3.0, label='Thermal expl.'),
               Line2D([], [], color='#d62728', lw=3.0, label='Event loiter'),
               Line2D([], [], color='#2ca02c', lw=2.0, label='Glide'),
               Line2D([], [], color='#7f7f7f', lw=2.0, label='Landing'),
               Line2D([], [], marker='*', color='w', mfc='#d62728', mec='k', ms=12, label='HP (inv.)'),
               patches.Patch(facecolor='orange', alpha=0.2, ec='#d97706', label='Thermal')]
    ax.legend(handles=handles, fontsize=12, ncol=2, loc='upper left')
    ax.set_xlabel("East (m)"); ax.set_ylabel("North (m)"); ax.set_aspect('equal')
...
def baselines():
    ...
    for m, yl, nm in [("final_soc_pct", "Final SOC (%)", "baseline_final_soc"),
        ...
        fig, ax = newfig(2.6, w=3.0)  # slightly wider for text
        ax.bar(xs, d[m + "_mean"].to_numpy(), yerr=d[m + "_std"].fillna(0).to_numpy(),
               capsize=4, color='#4477aa', edgecolor='k', alpha=0.85)
        ax.set_xticks(xs); ax.set_xticklabels(lab, fontsize=12, rotation=45, ha='right'); ax.set_ylabel(yl)
        savep(fig, nm)
...
def energy(run):
    ...
    fig, ax = newfig(2.4, w=3.0)
    ax.barh(range(len(eb)), eb["propulsion_energy_wh_mean"].to_numpy(),
            xerr=eb["propulsion_energy_wh_std"].fillna(0).to_numpy(), capsize=4,
            color=[MODE_COLORS[m] for m in eb["mode"]], edgecolor='k', alpha=0.85)
    ax.set_yticks(range(len(eb))); ax.set_yticklabels(eb["mode"], fontsize=12)
    ...
    for i, tr in traces.items():
        ...
        ax.plot(t, soc, color='lightgray', lw=1.0, zorder=1)
        for m in MODES:
            sel = (tr['fsm_state_name'] == m).to_numpy()
            ax.scatter(t[sel], soc[sel], s=4.0, color=MODE_COLORS[m], zorder=2)
    ax.set_xlabel("Time (s)"); ax.set_ylabel("SOC (%)")
    ax.legend(handles=[Line2D([], [], marker='o', ls='', color=MODE_COLORS[m], ms=5, label=m) for m in MODES],
    ...
    ax.bar(xs, bat["final_soc_pct_mean"].to_numpy(), yerr=bat["final_soc_pct_std"].fillna(0).to_numpy(),
           capsize=4, color=['#2ca02c', '#ff7f0e', '#7f7f7f'][:len(bat)], edgecolor='k', alpha=0.85)
    ax.set_xticks(xs); ax.set_xticklabels(["online\nestimator", "constant\npower", "PX4\nestimate"][:len(bat)], fontsize=14)
...
def thermal_sens():
    ...
    xs = np.arange(len(d))
    for m, yl, nm in [("thermal_encounters_per_uav", "Encounters / UAV", "thermal_encounters"),
                      ("thermalling_time_s", "Duration (s)", "thermal_duration"),
                      ("exploitation_saving_wh", "Energy saved (Wh)", "thermal_saving"),
                      ("final_soc_pct", "Final SOC (%)", "thermal_final_soc")]:
        fig, ax = newfig(2.4)
        sc = m + "_std"
        yerr = d[sc].fillna(0).to_numpy() if sc in d else None
        ax.errorbar(xs, d[m + "_mean"].to_numpy(), yerr=yerr, fmt='-o', color='#ee6677', capsize=5, lw=2.5, ms=8)
...
def thermal_trace(run):
    ...
    ax.plot(win['east_m'].to_numpy(), win['north_m'].to_numpy(), color='gray', lw=1.5)
    ax.plot(segp['east_m'].to_numpy(), segp['north_m'].to_numpy(), color='#ff7f0e', lw=3.0)
    ax.scatter(segp['east_m'].iloc[0], segp['north_m'].iloc[0], marker='^', s=100, c='#ff7f0e', ec='k', zorder=5)
    ax.scatter(segp['east_m'].iloc[-1], segp['north_m'].iloc[-1], marker='v', s=100, c='#1f77b4', ec='k', zorder=5)
    if len(segp):
    ...
    ax.set_xlabel("East (m)"); ax.set_ylabel("North (m)"); ax.set_aspect('equal')
    ax.legend(handles=[Line2D([], [], marker='^', color='w', mfc='#ff7f0e', mec='k', ms=10, label='entry'),
                       Line2D([], [], marker='v', color='w', mfc='#1f77b4', mec='k', ms=10, label='exit')], fontsize=14, loc='upper left')
    savep(fig, "thermal_trace_path")
    t = (win['sim_time'] - win['sim_time'].iloc[0]).to_numpy(); e0 = win['sim_time'].iloc[0]
    fig, ax = newfig(2.5)
    ax.plot(t, win['alt_rel_m'].to_numpy(), color='#1f77b4', label='Altitude (m)', lw=2.5)
    ax.axvspan(seg['entry_t'] - e0, seg['exit_t'] - e0, color='#ff7f0e', alpha=0.15)
    ax.set_xlabel("Time (s)"); ax.set_ylabel("Altitude (m)")
    ax2 = ax.twinx()
    ax2.plot(t, pd.to_numeric(win['energy_consumed_wh'], errors='coerce').to_numpy(), color='#d62728', lw=2.5, label='Energy (Wh)')
    ax2.plot(t, pd.to_numeric(win['wind_w_mps'], errors='coerce').to_numpy(), color='#2ca02c', lw=2.0, label='Updraft (m/s)')
    ax2.set_ylabel("Energy (Wh) / updraft (m/s)"); ax2.grid(False)
    l1, la1 = ax.get_legend_handles_labels(); l2, la2 = ax2.get_legend_handles_labels()
    ax.legend(l1 + l2, la1 + la2, fontsize=12, loc='upper center', bbox_to_anchor=(0.5, -0.15), ncol=2)
...
def events_fig():
    ...
    ax2 = ax.twinx()
    ax2.plot(xs, d["hp_detected_pct_mean"].to_numpy(), '-o', color='#1f77b4', lw=2.5, ms=8, label='Detected (%)')
    ax2.plot(xs, d["hp_investigated_pct_mean"].to_numpy(), '-s', color='#9467bd', lw=2.5, ms=8, label='Investigated (%)')
    ax2.set_ylabel("Percentage (%)"); ax2.grid(False)
    l1, la1 = ax.get_legend_handles_labels(); l2, la2 = ax2.get_legend_handles_labels()
    ax.legend(l1 + l2, la1 + la2, fontsize=11, loc='upper center', bbox_to_anchor=(0.5, -0.15), ncol=2)
    ...
    for xi, lvl in enumerate(d["level"]):
        ...
        if nlat >= 2:
            ...
            ax.errorbar([xi], [m], yerr=[[min(sd, m)], [sd]], fmt='o', color='#4477aa', capsize=6, ms=9, lw=2.5)
            ax.scatter([xi], [med], marker='_', s=250, color='#d62728', zorder=5)
        elif nlat == 1:
            ax.scatter([xi], [lats[0]], marker='o', s=80, color='#4477aa', zorder=5)
        ax.annotate(f"n={nlat}", (xi, 0), textcoords="offset points", xytext=(0, 5), fontsize=14, ha='center')
    ax.set_ylim(bottom=0); ax.set_xticks(xs); ax.set_xticklabels(d["level"]); ax.set_xlabel("Event load")
    ax.set_ylabel("HP latency (s)")
    ax.legend(handles=[Line2D([], [], marker='o', ls='', color='#4477aa', ms=8, label='mean ($\\pm$SD, $n\\geq2$)'),
                       Line2D([], [], marker='_', ls='', color='#d62728', ms=12, label='median')], fontsize=13, loc='upper center', bbox_to_anchor=(0.5, -0.15), ncol=2)
...
def stochastic():
    ...
    for m, yl, nm in [("route_completion_pct", "Route completion (%)", "stoch_route"),
        ...
        bp = ax.boxplot([vals], widths=0.5, showmeans=True, patch_artist=True,
                        meanprops=dict(marker='D', mfc='#d62728', mec='k', ms=8))
        for b in bp['boxes']:
            b.set(facecolor='#aac7e2', alpha=0.8, linewidth=2.0)
        for whisker in bp['whiskers']: whisker.set(linewidth=2.0)
        for cap in bp['caps']: cap.set(linewidth=2.0)
        for median in bp['medians']: median.set(linewidth=2.0)
        ax.set_xticks([1]); ax.set_xticklabels([f"R={len(vals)}"], fontsize=18); ax.set_ylabel(yl)
        savep(fig, nm)
...
def scalability():
    ...
    for m, s, yl, nm in [("cpu_pct_mean", "cpu_pct_std", "CPU usage (%)", "scal_cpu"),
        ...
        ax.errorbar(xs, d[m].to_numpy(), yerr=yerr, fmt='-o', color='#228833', capsize=5, lw=2.5, ms=8)
        if m == 'rtf_mean':
...
def landing():
    ...
    fig, ax = newfig(2.9)
    ax.add_patch(patches.Circle((0, 0), R, fill=False, ls='--', ec='#d62728', lw=2.0))
    for u, tr in traces.items():
        land = tr[tr['fsm_state_name'] == 'LANDING']
        if len(land) == 0:
            continue
        c = '#228833' if td.get(u) else '#cc3311'
        ax.plot(land['east_m'].to_numpy(), land['north_m'].to_numpy(), color=c, lw=2.0)
        ax.scatter(land['east_m'].iloc[-1], land['north_m'].iloc[-1], marker=('o' if td.get(u) else 'x'), s=60, c=c, zorder=5)
    ax.scatter([0], [0], marker='*', s=150, c='k', zorder=6)
    ax.set_xlabel("East (m)"); ax.set_ylabel("North (m)"); ax.set_aspect('equal')
    ax.legend(handles=[Line2D([], [], color='#228833', lw=2.0, label='touchdown ok'),
                       Line2D([], [], color='#cc3311', lw=2.0, label='failed'),
                       Line2D([], [], color='#d62728', ls='--', lw=1.5, label=f'zone R={R:.0f} m')], fontsize=11)
    ...
    for u, tr in traces.items():
        ...
        c = '#228833' if td.get(u) else '#cc3311'
        ax.plot(dist, land['alt_rel_m'].to_numpy(), color=c, lw=2.0)
    ax.axvline(R, color='#d62728', ls='--', lw=2.0); ax.axhline(3.0, color='gray', ls=':', lw=1.5)
    ax.invert_xaxis(); ax.set_xlabel("Distance to point (m)"); ax.set_ylabel("Altitude (m)")
    ...
    for row, u in enumerate(us):
        ...
        ax.plot([0, fa_t], [row, row], color='#4477aa', lw=5.0, solid_capstyle='butt')
        ax.plot([fa_t, end_t], [row, row], color='#ccbb44', lw=5.0, solid_capstyle='butt')
        ax.scatter([0], [row], marker='|', s=120, c='k', zorder=5)
        if tdi is not None and td.get(u):
            ax.scatter([t[tdi]], [row], marker='v', s=80, c='#228833', edgecolors='k', zorder=6)
        else:
            ax.scatter([end_t], [row], marker='x', s=80, c='#cc3311', zorder=6)
        ax.scatter([end_t], [row], marker=('s' if dis.get(u) else 'D'), s=50,
                   c=('#228833' if dis.get(u) else '#cc3311'), edgecolors='k', zorder=7)
    ax.set_yticks(range(len(us))); ax.set_yticklabels([f"UAV {u}" for u in us], fontsize=14)
    ...
    ax.legend(handles=[Line2D([], [], color='#4477aa', lw=3, label='approach/descent'),
                       ...
                       Line2D([], [], marker='v', ls='', mfc='#228833', mec='k', ms=6, label='touchdown'),
                       Line2D([], [], marker='x', ls='', color='#cc3311', ms=6, label='td failed'),
                       Line2D([], [], marker='s', ls='', mfc='#228833', mec='k', ms=6, label='disarm'),
                       Line2D([], [], marker='D', ls='', mfc='#cc3311', mec='k', ms=6, label='no disarm')],
              fontsize=9, ncol=2, loc='upper center', bbox_to_anchor=(0.5, -0.2))
