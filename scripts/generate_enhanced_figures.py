...
FIG = os.path.join(WS, "results/figure_enhanced_pdf")
...
COL_W = 2.5  # compact canvas: legible when shrunk into subfigures

plt.rcParams.update({
    'font.family': 'serif', 'font.size': 10, 'axes.labelsize': 10.5,
    'axes.titlesize': 10, 'xtick.labelsize': 9, 'ytick.labelsize': 9,
    'legend.fontsize': 8.5, 'lines.linewidth': 1.4, 'lines.markersize': 4.5,
...
def trajectories(run):
    ...
    ax.legend(handles=handles, fontsize=8.0, ncol=2, loc='upper left')
    ax.set_xlabel("East (m)"); ax.set_ylabel("North (m)"); ax.set_aspect('equal')
...
def baselines():
    ...
    for m, yl, nm in [("final_soc_pct", "Final SOC (%)", "baseline_final_soc"),
        ...
        ax.set_xticks(xs); ax.set_xticklabels(lab, fontsize=8.0, rotation=0); ax.set_ylabel(yl)
        savep(fig, nm)
...
def energy(run):
    ...
    ax.set_yticks(range(len(eb))); ax.set_yticklabels(eb["mode"], fontsize=9.6)
...
def thermal_trace(run):
    ...
    ax.legend(handles=[Line2D([], [], marker='^', color='w', mfc='#ff7f0e', mec='k', ms=6, label='entry'),
                       Line2D([], [], marker='v', color='w', mfc='#1f77b4', mec='k', ms=6, label='exit')], fontsize=9.6)
    ...
    ax.legend(l1 + l2, la1 + la2, fontsize=8.8, loc='center right')
...
def events_fig():
    ...
    l1, la1 = ax.get_legend_handles_labels(); l2, la2 = ax2.get_legend_handles_labels()
    ax.legend(l1 + l2, la1 + la2, fontsize=8.0, loc='upper left')
    ...
    for xi, lvl in enumerate(d["level"]):
        ...
        ax.annotate(f"n={nlat}", (xi, 0), textcoords="offset points", xytext=(0, 3), fontsize=9.6, ha='center')
    ...
    ax.legend(handles=[Line2D([], [], marker='o', ls='', color='#4477aa', label='mean ($\\pm$SD, $n\\geq2$)'),
                       Line2D([], [], marker='_', ls='', color='#d62728', label='median')], fontsize=8.8)
...
def stochastic():
    ...
    for m, yl, nm in [("route_completion_pct", "Route completion (%)", "stoch_route"),
        ...
        ax.set_xticks([1]); ax.set_xticklabels([f"R={len(vals)}"], fontsize=9.6); ax.set_ylabel(yl)
        savep(fig, nm)
...
def landing():
    ...
    ax.legend(handles=[Line2D([], [], color='#228833', lw=1.2, label='touchdown ok'),
                       ...
                       Line2D([], [], color='#d62728', ls='--', label=f'zone R={R:.0f} m')], fontsize=8.8)
    ...
    ax.set_yticks(range(len(us))); ax.set_yticklabels([f"UAV {u}" for u in us], fontsize=9.6)
    ...
    ax.legend(handles=[Line2D([], [], color='#4477aa', lw=3, label='approach/descent'),
              ...
              fontsize=7.7, ncol=2, loc='lower right')
